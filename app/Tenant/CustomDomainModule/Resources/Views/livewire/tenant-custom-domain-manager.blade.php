            ...
            <table class="min-w-full divide-y divide-zinc-200 text-sm dark:divide-zinc-700">
                ...
                <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                    @forelse ($domains as $domain)
                        <tr>
                            <td class="px-3 py-2 font-medium text-zinc-900 dark:text-zinc-100">{{ $domain->domain }}
                            </td>
                            <td class="px-3 py-2">
                                <span
                            ...
                            <td class="px-3 py-2">
                                @php
                                    $sslStatusClass = match ($domain->ssl_status) {
                                        'issued'
                                ...
                                @can('tenant.custom-domains.manage')
                                    <div class="flex flex-wrap gap-2">
                                        <flux:button size="xs" variant="ghost"
                                            wire:click="toggleVerification({{ $domain->id }})">
                                            {{ $domain->verified_at ? 'Marcar pendiente' : 'Marcar verificado' }}
                                        </flux:button>

                                        <flux:button size="xs" variant="primary"
                                            wire:click="requestSsl({{ $domain->id }})" :disabled="!$domain->verified_at">
                                            Solicitar SSL
                                        </flux:button>
                    ...
                    @empty
                        <tr>
                            <td class="px-3 py-6 text-center text-zinc-500" colspan="5">No hay dominios registrados.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
