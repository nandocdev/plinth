        ...
        <div class="rounded-xl border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <form wire:submit="{{ $editingPlanId ? 'updatePlan' : 'createPlan' }}"
                ...
                <flux:input wire:model="planForm.slug" :label="__('Slug')" :placeholder="__('pro-monthly')"
                    required />
                <flux:input wire:model="planForm.priceMonthlyCents" type="number"
                    :label="__('Precio mensual (centavos)')" min="1" required />
                            ...
                            <tr class="border-b border-zinc-100 dark:border-zinc-800"
                                ...
                                <td class="py-3 text-right">
                                    <div class="flex justify-end gap-2">
                                        <flux:button wire:click="startPlanEditing({{ $plan->id }})"
                                            variant="filled" size="sm">
                                            {{ __('Editar') }}
                                        </flux:button>
                                        <flux:button wire:click="deletePlan({{ $plan->id }})"
                                            wire:confirm="{{ __('Esto eliminara el plan. Continuar?') }}"
